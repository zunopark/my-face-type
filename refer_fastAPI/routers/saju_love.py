...
def build_love_prompt(saju_data: Dict[str, Any], user_name: str = None, user_concern: str = None, year: int = 2026) -> str:
    ...
    prompt = f"""
        ...
        - 이성이 {name}님을 처음 봤을 때 느끼는 분위기와 인상을 생생하게 묘사해요.
        ...
        - 사주에서 보이는 {name}님이 끌리는 스타일과, 반대로 {name}님에게 끌리는 사람의 특징을 분석해요.
        ...
        - {name}님 사주의 장점을 살리고 약점을 보완하는 연애 전략을 제안해요.

        ---

        ##[4장] 색동낭자의 일침

        **{name}님의 고민: "{concern_for_prompt}"**

        **[작성 지침 - 소제목 없이 하나의 흐름으로 작성]**
        이 장은 ###풀이 1, ###풀이 2 같은 소제목 없이 자연스러운 대화체로 작성해요.
        마치 {name}님과 1:1로 마주 앉아 진심어린 상담을 하는 것처럼요.

        **[흐름]**
        1. 고민 인용으로 시작 → "'{concern_for_prompt}'라고 하셨죠."
        2. 공감하되 바로 본론으로 → 사주에서 보이는 근본 원인을 단도직입적으로 짚기
        3. 뼈 때리는 직언 → 듣기 싫어도 {name}님을 위해 솔직하게 말하기
        ...
        **[문체 예시]**
        "{name}님, '{concern_for_prompt}'라고 하셨죠. 사주 보면서 왜 이런 고민이 생겼는지 바로 보여요.

        솔직히 말씀드릴게요. 이건 상대방 문제가 아니에요. {name}님 사주에 OO가 강해서...

        냉정하게 들릴 수 있는데, {name}님 일지의 OO 때문에 무의식적으로 OO하는 패턴이 있어요. 이게 계속 같은 상황을 만드는 거예요.

        그래서 이번 주부터 이렇게 해보세요. 연락 올 때 OO분은 기다렸다가 답장하고, 다음 만남에서 OO 얘기는 절대 먼저 꺼내지 마세요.

        하지만 {name}님 사주에 OO가 있어서, 이것만 고치면 정말 좋은 인연 만날 수 있어요. {name}님처럼 OO한 사람은 드물거든요."

        ---

        **중요**: 위의 모든 내용을 바탕으로, 친근하지만 단호한 전문가의 어조로 {name}님만을 위한 맞춤형 연애 사주 풀이를 작성해 주세요. 특히 4장은 소제목 없이 자연스러운 1:1 상담 대화체로 작성해 주세요.
        """
